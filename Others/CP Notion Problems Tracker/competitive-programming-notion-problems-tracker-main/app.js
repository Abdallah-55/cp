async function main() {
      ...
      switch (platform.name) {
        case codeforces_name:
          ...
          problem_url = `https://codeforces.com/${
            contest_id > codeforces_id_threshold ? "gym" : "contest"
    ...
    static generateUrl(platform, problem, id) {
      ...
      switch (platform.name) {
        case codeforces_name:
          ...
          submission_url = `https://codeforces.com/${
            contest_id > codeforces_id_threshold ? "gym" : "contest"
    ...
    async getCode(platform) {
      switch (platform.name) {
        case codeforces_name:
          ...
          return cheerio
            .load((await axios.get(this.url)).data)("#program-source-text")
            .text();
        case vjudge_name:
          await sleep(sleep_duration.vjudge_name);
          return (
            await axios.get(`https://vjudge.net/solution/data/${this.id}`)
          ).data.codeAccessInfo; // FIXME: Take into account the actual text while logged in
        case leetcode_name:
          return this.code;
      }
    }
  ...
  class Platform {
    ...
    static getMaxUrlLength(platform_name) {
      ...
      maxUrlLength.problem += extra;
      maxUrlLength.submission += extra;
      return maxUrlLength;
    }
    ...
    static parseCookie(cookies_raw, name = "csrftoken") {
      const cookies = cookies_raw.headers["set-cookie"][0]
        .split(";")
    ...
    async fetchData() {
      ...
      switch (this.name) {
        case codeforces_name:
          ...
          submissions = await axios.get(
            `${this.base_url}/${fetchRequestParams}&apiSig=${randomSig}${crypto
              .createHash("sha512")
              .update(
                `${randomSig}/${fetchRequestParams}#${process.env.CODEFORCES_SECRET}`
              )
              .digest("hex")}`
          );
          return submissions.data.result.filter(
            (submission) =>
              submission.creationTimeSeconds > this.last_submission_timestamp
          );
        case vjudge_name:
          submissions = [];
          let i = 0;
          while (true) {
            await sleep(sleep_duration.vjudge_name);
            let currentSubmissions = await axios.get(
              `${this.base_url}/status/data?start=${
                i * vjudge_max_results_length
              }&length=${vjudge_max_results_length}&un=${process.env.VJUDGE_ID}`
            );
          ...
          return submissions.filter(
            (submission) => submission.time > this.last_submission_timestamp
          );
        case leetcode_name:
          let leetcode_headers = Platform.generateLeetcodeHeaders(
            await Platform.getCSRF(leetcode_name)
          );
          let has_next = true;
          let offset = 0;
          while (has_next) {
            ...
            const response = await axios(
              `${this.base_url}/api/submissions?offset=${offset}&limit=${leetcode_limit}`,
              {
            ...
            leetcode_headers = Platform.generateLeetcodeHeaders(
              Platform.parseCookie(response)
            );
          }
          return submissions.filter(
            (submission) =>
              submission.timestamp > this.last_submission_timestamp
          );
      }
    }
      ...
      switch (this.name) {
        ...
        case vjudge_name:
          ...
          name = await axios.get(
            `${this.base_url}/problem/data?start=0&length=1&OJId=${data.oj}&probNum=${data.probNum}&title=&source=&category=`
          );
        ...
        case leetcode_name:
          ...
          const problem_info = await axios.post(`${this.base_url}/graphql`, {
            headers: Platform.generateLeetcodeHeaders(
              Platform.getCSRF(leetcode_name)
            ),
            query: leetcode_query_get_problem,
            variables: { titleSlug: data.title_slug },
          });
          difficulty = problem_info.data.data.question.difficulty;
          for (const tag of problem_info.data.data.question.topicTags)
    ...
    cleanSubmissions(submissions_data, problems) {
      ...
      for (let i = 0; i < problems.length; i++) {
        ...
        submissions.push(
          new Submission(id, url, timestamp, verdict, language, problem, code)
        );
      }
      return submissions;
    }
  ...
  class Entry {
    ...
    static cleanEntry(entry) {
      ...
      const id =
        properties["Problem ID"].rich_text.length > 0
          ? properties["Problem ID"].rich_text[0].plain_text
          : null;
      if (!id) return null;
      const url = properties.URL.url;
      const name =
        properties["Problem Name"].title.length > 0
          ? properties["Problem Name"].title[0].plain_text
          : null;
      if (!name) return null;
      const tags = properties.Tags.multi_select;
      for (let i = 0; i < tags.length; i++) tags[i] = tags[i].name;
      const platform = properties.Platform.select.name;
      const difficulty = properties["Difficulty (Raw)"].rich_text[0].plain_text;
      const languages_accepted = properties["Languages Accepted"].multi_select;
      for (let i = 0; i < languages_accepted.length; i++)
        languages_accepted[i] = languages_accepted[i].name;
      const languages_attempted =
        properties["Languages Attempted"].multi_select;
      for (let i = 0; i < languages_attempted.length; i++)
        languages_attempted[i] = languages_attempted[i].name;
      return new Entry(
        page_id,
        id,
        url,
  ...
  class Notion {
    ...
    async getUser() {
      return (await this.notion.users.list()).results.find(
        ({ type }) => type == "person"
      ).name;
    }
    ...
    async fetchEntries(platform) {
      ...
      while (has_more) {
        ...
        const query = await this.notion.databases.query({
          database_id: this.NOTION_DATABASE_ID,
          filter: { property: "Platform", select: { equals: platform.name } },
    ...
    async updateDB(NOTION_DB, platform, submissions) {
      ...
      for (let submission of submissions) {
        ...
        if (!entries[problem.id]) {
          entries[problem.id] = Entry.cleanEntry(
            await this.createPage(platform, problem)
          );
        ...
        if (!entry.languages_attempted.includes(submission.language)) {
          await this.updateAttemptedLanguages(entry.page_id, [
            ...entry.languages_attempted,
            submission.language,
          ]);
        }
        console.log(
          `\t\t\t[PENDING] ${submission.language} solution for ${problem}`
        );
        ...
        if (submission.verdict == "accepted") {
          ...
          if (
            !entries[problem.id].languages_accepted.includes(
              submission.language
            )
          ) {
            await this.updateAcceptedLanguages(entry.page_id, [
              ...entry.languages_accepted,
              submission.language,
            ]);
          }
          console.log(
            `\t\t\t[ACCEPTED] ${submission.language} solution for ${problem} `
          );
          ...
          console.log(
            `\t\t\t[REJECTED] ${submission.language} solution for ${problem}`
          );
        }
      }
  ...
  async function getRandomProxy(protocol = "https") {
    ...
    for (let i = 0; i < proxies.length; i++)
      proxies[i] = proxies[i].substring(0, proxies[i].lastIndexOf(":"));
  ...
  async function randomWait() {
    return new Promise((resolve) =>
      setTimeout(resolve, generateRandomNumber(1001))
    );
  }
  ...
  try {
    ...
    const codeforces = new Platform(
      codeforces_name,
      process.env.CODEFORCES_ID,
      Number(process.env.CODEFORCES_LAST_SUBMISSION_TIMESTAMP),
      process.env.CODEFORCES_KEY,
      process.env.CODEFORCES_SECRET
    );
    const leetcode = new Platform(
      leetcode_name,
      process.env.LEETCODE_ID,
      Number(process.env.LEETCODE_LAST_SUBMISSION_TIMESTAMP)
    );
    const vjudge = new Platform(
      vjudge_name,
      process.env.VJUDGE_ID,
      Number(process.env.VJUDGE_LAST_SUBMISSION_TIMESTAMP)
    );
    ...
    for (let platform of platforms) {
      ...
      for (let data of submissions_data) {
        ...
        console.log(
          `\tFetched from ${problem.url.padEnd(
            platform.max_url_length.problem,
            " "
          )} ${problem}`
        );
      ...
      for (let submission of submissions) {
        console.log(
          `\tFormatted from ${submission.url.padEnd(
            platform.max_url_length.submission,
            " "
          )} ${submission.problem}`
        );
      }
